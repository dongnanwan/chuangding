<template>
    <div>
        ...
        <div class="mytable" v-if="isShow">
            <input type="text" v-model="inputShopName" v-on:keyup="searchShopName" placeholder="请输入负责人名字" class="input">
            <i class="el-icon-close" @click='closeDialog'></i>
            <el-table :data="shopNameOptions" style="width: 100%" @row-click='getRowDetail'  v-loading="loading">
                ...
                <el-table-column show-overflow-tooltip prop="roles.name" label="角色" width="180">
...
import { get } from 'http';
  ...
  export default {
    ...
    methods: {
        ...
        searchShopName(){
            this.$axios.get(`/api/user?name=${this.inputShopName}`,{params:this.params}).then((res)=>{
                this.shopNameOptions = res.data.data.data
                this.total = res.data.data.total
                // console.log(res.data.data.data)
            })
    ...
    mounted(){
        this.getInfo()
        console.log(this.mangerName)
        this.shopName = this.mangerName
