<template>
  <div>
      ...
      <div>
          ...
          <div style="margin-bottom:10px">
            ...
            <el-select v-model="params.status" placeholder="状态">
              ...
              <el-option value="FINISHED" label="已付款 拼团中"></el-option>
              <el-option value="TUAN_ING" label="申请退款"></el-option>
      ...
      <el-table-column show-overflow-tooltip prop="userName" label="客服" min-width="80"></el-table-column>
      ...
      <el-table-column show-overflow-tooltip fixed="right" label="操作" width="80">
        <template slot-scope="scope">
          <!-- <el-button type="primary" plain @click="showDetail(scope.$index,scope.row)">详情</el-button> -->
          <el-button  :disabled="scope.row.userName != '未分配'" type="primary" plain @click="showDispatchTask(scope.$index,scope.row)">分配</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="block">
  ...
  export default {
    ...
    methods:{
      ...
      getInfo(){
        ...
        this.$axios.get('/api/customer-service/trade-user',{params:this.params}).then(res=>{
      ...
      showDetail(index, row) {
        // console.log(row)
          ...
          this.gridData = []
          this.$axios.get(`/api/customer-service/trade-user/${row.id}`).then(res=>{
      ...
      showDispatchTask(index,row) {
        this.$confirm('是否确认?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.dispatchTask(index,row)
        }).catch(() => {
                  
        });
      },
      dispatchTask(index,row){
        this.$axios.post(`/api/customer-service/distribute?tradeId=${row.id}`).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
