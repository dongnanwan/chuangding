  ...
  <div>
    <div style="padding-bottom:10px">
      ...
      >
          ...
          <el-table-column show-overflow-tooltip prop="summaryTime" label="月份">
            <template slot-scope="scope">
              <span>{{scope.row.selectYear+'-'+scope.row.selectMonth}}</span>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip label="天数">
            <template slot-scope="scope">
              {{new Date(scope.row.selectYear, scope.row.selectMonth, 0).getDate()}}
            </template>
  ...
  export default {
    ...
    methods:{
      ...
      getInfo(){
        ...
        this.$axios.get('/api/getMonthlyWorkSummary',{params:this.params}).then(res=>{
