  ...
  <div>
    <div style="padding-bottom:10px">
      ...
      >
          ...
          <el-table-column show-overflow-tooltip  label="每周开始时间">
            <template slot-scope="scope">
              <span>{{new Date(parseInt(scope.row.startOfWeek)).toLocaleString()}}</span>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip  label="每周结束时间">
            <template slot-scope="scope">
              <span>{{new Date(parseInt(scope.row.endOfWeek)).toLocaleString()}}</span>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip prop="" label="天数">7</el-table-column>
  ...
  export default {
    ...
    methods:{
      ...
      getInfo(){
        ...
        this.$axios.get('/api/getWeeklyWorkSummary',{params:this.params}).then(res=>{
