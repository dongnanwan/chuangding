<template>
  <div>
    <div style="padding-bottom:10px">
        ...
        <br>
        <el-input v-model="inputShopName" v-on:keyup.native="searchShopName" placeholder="请输入合伙人名称" style="margin-top:10px;width:300px"></el-input>
    </div>
    ...
    <el-table :data="items" height="600" style="width: 100%"  border highlight-current-row @selection-change="handleSelectionChange" v-loading="indexLoading" element-loading-text="数据加载中......"
        ...
        <el-table-column prop="name" label="合伙人" min-width="150"></el-table-column>
  ...
  export default {
    ...
    methods:{
        ...
        unbindWechart(index,row){
            this.$axios.post(`/api/partner/unbind/${row.id}`).then((res)=>{
        ...
        submitCreateNew() {
            this.$axios.post('/api/partner/add',this.form).then((res)=>{
                if(res.data.code == 200){
                    this.msgCreate = '操作成功'
                    this.msgType = 'success'
                    this.msgCreateOpen()
