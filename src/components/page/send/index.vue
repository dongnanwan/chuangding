      ...
      <div style="height:40px">
          <div style="float:left">
            ...
            <div style="margin:10px 0">
              ...
              <div style="float:left">
                <ShopIdInputx-app @OnGetDataFromChild='GetShopIdFromChildSearch' :fatherData='fatherList'></ShopIdInputx-app>
              </div>
          ...
          <el-table-column show-overflow-tooltip label="操作" width="275" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                type="danger"
                plain
                @click="showRollback(scope.$index, scope.row)">驳回</el-button> 
...
<script>
  ...
  export default {
    ...
    methods:{
      rollback(index,row){
        this.$axios.put(`/api/send/${row.code}/rollback`).then(res=>{
      ...
      showRollback(index,row) {
        this.$confirm('是否确认驳回?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.rollback(index,row)
        }).catch(() => {
           
        });
      },
      doHandle(){
        this.$axios.put(`/api/send/${this.handleCode}/deal?skuIds=${[this.handleSkuId]}&responsiblePerson=${this.handleForm.responsiblePerson}&responsibleAmount=${this.handleForm.responsibleAmount}&remark=${this.handleForm.remark}&differenceType=${this.handleForm.differenceType}`).then(res=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
