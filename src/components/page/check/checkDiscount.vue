<template>
  <div>
      ...
      <el-table :data="items" height="600" style="width: 100%"  border highlight-current-row @selection-change="handleSelectionChange" v-loading="indexLoading" element-loading-text="数据加载中......"
          ...
          <el-table-column show-overflow-tooltip prop="startTime" label="开始时间" min-width='100'></el-table-column>
          <el-table-column show-overflow-tooltip prop="endTime" label="结束时间" min-width='100'></el-table-column>
          <el-table-column show-overflow-tooltip prop="" label="折扣商品" min-width='100'>
            <template slot-scope="scope">
               <span>{{getAllItemName(scope.row.items)}}</span>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip prop="shopId" label="折扣门店" min-width='100'>
              <template slot-scope="scope">
                  <span v-if="scope.row.shopId === 'ALL'">全部门店</span>
                  <span v-else v-for="item in scope.row.shops" :key="item.id">
                    {{item.name+','}}
                  </span>
              </template>
          </el-table-column>
          ...
          <el-table-column show-overflow-tooltip  label="客户类型" min-width='100'>
              <template slot-scope="scope">
                  ...
                  <!-- <span v-else>{{scope.row.customerType}}</span> -->
              </template>
          </el-table-column>
          ...
          <!-- <el-table-column show-overflow-tooltip label="提交活动" min-width='100'>
            <template slot-scope="scope">
          ...
          <el-table-column show-overflow-tooltip label="审核" width='100'>
            <template slot-scope="scope">
  ...
  export default {
    ...
    methods:{
       ...
       console.log(row)
          ...
          if (row.items.length > 0){
            for(let i = 0;i < row.items.length;i ++){
              this.$axios.get(`/api/activity/item-detail/${row.items[i].id}`,{params:{activityType:'DISCOUNT'}}).then(res=>{
                  // console.log(res)
      ...
      updateDetail(index,row){
        // console.log(row.id)
        this.updateForm = {type:'DISCOUNT',id:row.id,title:row.title,startTime:row.startTime,endTime:row.endTime,customerType:row.customerType,couponsAllowed:row.couponsAllowed.toString(),shareBenefit:row.shareBenefit.toString()}
      ...
      submitCreateNew() {
        //  console.log(this.form)
         this.$axios.post(`/api/activity/save-discount-activity`,this.form).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
      ...
      submitUpdate(){
        this.$axios.put(`/api/activity/modified-discount`,this.updateForm).then((res)=>{
          if(res.data.code == 200){
            this.items = res.data.data.data
            this.total = res.data.data.total
            this.indexLoading = false
