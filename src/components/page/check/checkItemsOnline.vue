<template>
    <div>
        ...
        <el-table
            ...
            height="600"
            >
            <el-table-column prop="id" label="ID" sortable  show-overflow-tooltip  min-width="60">
                <!-- <template slot-scope="scope">
                    <div v-if="scope.row.goldDiscount != 591" style="background:red">{{scope.row.id}}</div>
                    <div v-else>{{scope.row.id}}</div>
                </template> -->
            </el-table-column>

            <el-table-column prop="goldDiscount" label="金卡折扣"  show-overflow-tooltip  min-width="100">
                <template slot-scope="scope">
                    <div v-if="scope.row.goldDiscount != 595" style="background:red;color:white">{{scope.row.goldDiscount}}</div>
                    <div v-else>{{scope.row.goldDiscount}}</div>
                </template>
            </el-table-column>
            ...
            <el-table-column label="副标题"  show-overflow-tooltip min-width="240" prop="subTitle"></el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="267" show-overflow-tooltip>
                <template slot-scope="scope"> 
                    <el-button plain="" type="primary" @click="showEditDialogue(scope.$index, scope.row)" style="margin:0 5px">详情</el-button>
                    <el-button  plain="" type="success" @click="itemOnline(scope.$index, scope.row)" style="margin:0 5px">同意上架</el-button>
                    <el-button  plain="" type="danger" @click="rejectOnline(scope.$index, scope.row)" style="margin:0 5px">拒绝上架</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods:{
        ...
        rejectOnline(index,row){
            this.$axios.put(`/api/item/verify/reject?id=${row.id}`).then(res=>{
                if(res.data.code == 200){
                    this.msgCreate = '操作成功'
                    this.msgType = 'success'
                    this.msgCreateOpen()
