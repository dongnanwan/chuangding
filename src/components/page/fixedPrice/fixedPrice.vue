<template>
  <div>
      ...
      <el-table :data="items"  height="600" style="width: 100%"  border highlight-current-row @selection-change="handleSelectionChange" v-loading="indexLoading" element-loading-text="数据加载中......"
          ...
          <el-table-column show-overflow-tooltip prop="" label="一口价商品"  min-width="100">
      ...
      <el-dialog   :visible.sync="dialogTableVisible" class="detailTable"  fullscreen>
         ...
         <el-card >
            ...
            >
              ...
              <el-table-column show-overflow-tooltip prop="activityPrice" label="促销价"></el-table-column>
        ...
        <!-- 添加商品。。。。。。。。。。。。。。。。。。。。 -->
              ...
              <el-table border :data="skuTableData">
                    ...
                    <el-table-column label="价格" >
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.getInputPrice" placeholder="请输入价格" ></el-input>
                        </template>  
                    </el-table-column>

                    <el-table-column label="限量" >
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.getInputRestStock" placeholder="请输入数量"></el-input>
                        </template>  
                    </el-table-column>
  ...
  export default {
    ...
    methods:{
      ...
      showDetail(index, row) {  
          ...
          if (row.items.length > 0){
            for(let i = 0;i < row.items.length;i ++){
              this.$axios.get(`/api/activity/item-detail/${row.items[i].activityId}/${row.items[i].id}`,{params:{activityType:'FIXED_PRICE'}}).then(res=>{
                  // console.log(res)
      ...
      updateDetail(index,row){
        // console.log(row.id)
        this.updateForm = {type:'FIXED_PRICE',id:row.id,title:row.title,startTime:row.startTime,endTime:row.endTime,customerType:row.customerType,couponsAllowed:row.couponsAllowed.toString(),shareBenefit:row.shareBenefit.toString()}
      ...
      submitCreateNew() {
        //  console.log(this.form)
         this.$axios.post(`/api/activity/save-fixed-price-activity`,this.form).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
      ...
      submitUpdate(){
        this.$axios.put(`/api/activity/modified-fixed-price`,this.updateForm).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
