  ...
  export default {
    ...
    methods:{
      ...
      showDetail(index, row) {  
          ...
          if (row.items.length > 0){
            for(let i = 0;i < row.items.length;i ++){
              this.$axios.get(`/api/activity/item-detail/${row.items[i].activityId}/${row.items[i].id}`,{params:{activityType:'SECKILL'}}).then(res=>{
                  // console.log(res)
      ...
      updateDetail(index,row){
        // console.log(row.id)
        this.updateForm = {type:'SECKILL',id:row.id,title:row.title,startTime:row.startTime,endTime:row.endTime,customerType:row.customerType,couponsAllowed:row.couponsAllowed.toString(),shareBenefit:row.shareBenefit.toString()}
      ...
      submitCreateNew() {
        //  console.log(this.form)
         this.$axios.post(`/api/activity/save-seckill-activity`,this.form).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
      ...
      submitUpdate(){
        ...
        this.$axios.put(`/api/activity/modified-seckill`,this.updateForm).then((res)=>{
          if(res.data.code == 200){
            this.msgCreate = '操作成功'
            this.msgType = 'success'
            this.msgCreateOpen()
