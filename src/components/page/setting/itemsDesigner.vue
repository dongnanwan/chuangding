...
export default {
    ...
    methods:{
        setItemsRecommend(){
            ...
            this.$axios.post(`/api/recommend/item/add?recommendType=recommend_designer_items&items=${value}`).then((res)=>{
        ...
        getInfo(){
            this.indexLoading = true
            this.$axios.get('/api/recommend/items?recommendType=recommend_designer_items',{params:this.params}).then(res=>{
                if(res.data.code == 200){
