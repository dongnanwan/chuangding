  ...
  <div>
    <div style="height:40px">
      ...
      >
          ...
          <el-table-column show-overflow-tooltip fixed="right" label="操作" width="335">
            <template slot-scope="scope">
              ...
              <el-button
                :disabled="scope.row.transitStatus == 'Wait_Receive'"
                size="mini"
                type="primary"
                plain
                @click="createSender(scope.$index, scope.row)">生成发货单</el-button>
              <el-button
                size="mini"
                type="danger"
                plain
                @click="showCancle(scope.$index, scope.row)">取消</el-button>
            </template>
          </el-table-column>
      ...
      <el-dialog  :visible.sync="createTableVisible" class="detailTable"  fullscreen>
            ...
            <el-form ref="form" :model="form" label-width="80px" :inline="false"  class="demo-form-inline">
                <div style="height:50px">
                    ...
                    <div style="position:absolute">
                        <ShopIdInput-app componentName='发货店铺' @OnGetDataFromChild='GetSenderFromChild'></ShopIdInput-app>
                    </div>
                </div>
                ...
                <div style="height:50px">
                    ...
                    <div style="position:absolute">
                        <ShopIdInput-app componentName='收货店铺' @OnGetDataFromChild='GetReceiverFromChild'></ShopIdInput-app>
                    </div>
                </div>
        ...
        <el-card shadow="hover" style="margin-top:10px">
            <div slot="header" class="clearfix" style="height:25px"> 
              ...
              <SkuInput-app :fatherData='senderShopId' @onInsertSkuData='insertSkuData'></SkuInput-app> 
...
<script>
  ...
  import SkuInputVue from '../../common/stockSelect.vue';
  ...
  export default {
    ...
    methods:{
      showCancle(index,row) {
        this.$confirm('是否确认?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.cancelThis(index,row)
        }).catch(() => {
                
        });
      },
      cancelThis(index,row){
        this.$axios.put(`/api/allocate/${row.code}/cancel`).then(res=>{
