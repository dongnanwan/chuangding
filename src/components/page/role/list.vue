  ...
  <div>
    <div style="height:40px">
      <div style="float:left">
        ...
        <el-button type="primary" plain="" @click="addOne()" icon="el-icon-plus">添加</el-button>
      </div>
    </div>
    ...
    >
      ...
      <el-table-column show-overflow-tooltip label="操作" width="195">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" plain="" @click="showRoleTable(scope.$index,scope.row)">配置角色权限</el-button>
          <el-button type="danger" size="mini" plain="" @click="showDelete(scope.$index,scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
    ...
    methods:{
        ...
        add(){
          this.$axios.post(`/api/role/add?name=${this.roleName}`).then(res=>{
              if(res.data.code == 200){
                  this.msgCreate = '操作成功'
                  this.msgType = 'success'
                  this.msgCreateOpen()
